...
const MyProfile = ({ userRole, userId, userData, onProfileUpdate }) => {
  ...
  const uploadToIPFS = async (file) => {
    ...
    const res = await axios.post(`${API_BASE_URL}/upload/ipfs`, fd, {
      headers: { "Content-Type": "multipart/form-data" }
    });
    return res.data.ipfs_hashes ? res.data.ipfs_hashes[0] : res.data.ipfs_hash;
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (userRole === "PATIENT") {
        await axios.put(`${API_BASE_URL}/update/patient/${userId}`, updatePayload);
      } else if (userRole?.includes("DOCTOR")) {
        if (certHash) updatePayload.certificate_hash = certHash;
        await axios.put(`${API_BASE_URL}/update/doctor/${userId}`, updatePayload);
      }
